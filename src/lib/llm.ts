  ...
  try {
    if (!process.env.OPENAI_API_KEY) {
      return {
        ...
        explanation: "",
        error:
          "OpenAI API key is not configured. Set OPENAI_API_KEY in your environment to enable SQL generation.",
      };
    }
...
7. Return the query in a clean, formatted way`;
    ...
    if (status === 429 || /quota|rate limit/i.test(msg)) {
      return {
        sql: "",
        explanation: "The SQL generator hit a rate limit or quota.",
        error:
          "OpenAI: 429 quota/rate limit exceeded. Please check billing/plan or try again later.",
      };
    }
    if (status === 401 || /unauthorized|api key/i.test(msg)) {
      return {
        sql: "",
        explanation: "",
        error:
          "OpenAI API key invalid or missing. Set OPENAI_API_KEY and restart the server.",
      };
    }
    return {
      sql: "",
