...
7. Return the query in a clean, formatted way`;
    ...
    let cleanSQL = generatedSQL
      // strip fenced code blocks with or without language label (sql, sqlite, postgresql, etc.)
      .replace(/```[a-zA-Z]*\n?/g, "")
      .replace(/```\n?/g, "")
      .trim();

    // Some models may prepend stray characters or notes. Extract the first SQL statement
    // starting with SELECT or WITH and keep until the first semicolon (inclusive) if present.
    const sqlStart = cleanSQL.search(/\b(select|with)\b/i);
    if (sqlStart > 0) {
      cleanSQL = cleanSQL.slice(sqlStart);
    }
    const semiIdx = cleanSQL.indexOf(";");
    if (semiIdx !== -1) {
      cleanSQL = cleanSQL.slice(0, semiIdx + 1);
    }
...
export function validateSQL(sql: string): { valid: boolean; error?: string } {
  ...
  if (!/^\s*(select|with)\b/.test(lowerSQL2)) {
    return {
      valid: false,
